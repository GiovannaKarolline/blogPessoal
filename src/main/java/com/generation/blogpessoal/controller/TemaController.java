...
public class TemaController {
	...
	//Entidade de resposta HTTP (ResponseEntity):
	@GetMapping
	...
	public ResponseEntity<List<Tema>> getAllByDescricao(@PathVariable String descricao){
		...
		// o mesmo que SELECT * FROM tb_temas WHERE descricao LIKE "%?%";
	}
	
	@PostMapping
	public ResponseEntity<Tema> post(@Valid @RequestBody Tema tema){
		return ResponseEntity.status(HttpStatus.CREATED).body(temaRepository.save(tema));
		// o mesmo que INSERT INTO tb_temas(descricao) VALUES(?);
	}
	...
	public ResponseEntity<Tema> put(@Valid @RequestBody Tema tema){
		...
		if(temaRepository.existsById(tema.getId()))
			...
			// o mesmo que UPDATE tb_temas SET descricao = ? WHERE id = ?;
	...
	public void delete(@PathVariable Long id){
		...
		// o mesmo que DELETE FROM tb_temas WHERE id = ?;
	}
}
